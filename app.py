...
if uploaded_raw_data and uploaded_hc_data:
    try:
        # --- NEW: Robust file reading function ---
        def read_uploaded_file(file):
            """Tries to read an uploaded file as either Excel or CSV with correct encoding."""
            try:
                # First, try to read as an Excel file
                return pd.read_excel(file)
            except Exception as e1:
                st.warning(f"لم يتمكن من قراءة '{file.name}' كملف إكسل. يحاول الآن قراءته كملف CSV... (الخطأ: {e1})")
                # If Excel fails, rewind the file and try reading as CSV
                file.seek(0)
                try:
                    # Try reading as CSV with the correct encoding for Arabic
                    return pd.read_csv(file, encoding='windows-1256', sep=',', on_bad_lines='skip')
                except Exception as e2:
                    st.error(f"فشل في قراءة الملف '{file.name}' كملف إكسل أو CSV. (الخطأ: {e2})")
                    return None
        ...
        for file in uploaded_raw_data:
            df_single_raw = read_uploaded_file(file)
            if df_single_raw is not None:
                all_raw_data_dfs.append(df_single_raw)

        if not all_raw_data_dfs:
            st.error("لم يتم قراءة أي من ملفات البيانات الخام بنجاح.")
        else:
            ...
            df_hc = read_uploaded_file(uploaded_hc_data)
