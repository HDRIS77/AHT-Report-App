...
def calculate_all_metrics(df_raw_overall, df_raw_urdu, target_aht):
    ...
    for tl in df_raw_overall['AD'].unique():
        ...
        # إضافة مقاييس البلدان
        for country in countries:
            metrics[tl][country.lower()] = country_metrics[country].get(tl, 0)
    
    return metrics

def create_full_report(metrics, target_aht, output_path):
    """
    ...
    for tl, data in metrics.items():
        ...
        # Overall AHT Score
        view_sheet.cell(row=row_num, column=4, value=data['overall_aht'] if data['overall_aht'] > 0 else "-")
        
        # Tenured AHT
        view_sheet.cell(row=row_num, column=5, value=data['tenured_aht'] if data['tenured_aht'] > 0 else "-")
        
        # Nesting AHT
        view_sheet.cell(row=row_num, column=6, value=data['nesting_aht'] if data['nesting_aht'] > 0 else "-")
        ...
        # Var From Target
        view_sheet.cell(row=row_num, column=9, value=data['var_from_target'] if data['var_from_target'] != 0 else "-")
        ...
        # Readiness
        view_sheet.cell(row=row_num, column=11, value=f"{data['readiness']:.2f}%" if data['readiness'] > 0 else "-")
        
        # FRT
        view_sheet.cell(row=row_num, column=12, value=data['frt'] if data['frt'] > 0 else "-")
        
        # Countries (EG, JO, BH, AE, QA, KW, OM)
        view_sheet.cell(row=row_num, column=13, value=data['eg'] if data['eg'] > 0 else "-")  # EG
        view_sheet.cell(row=row_num, column=14, value=data['jo'] if data['jo'] > 0 else "-")  # JO
        view_sheet.cell(row=row_num, column=15, value=data['bh'] if data['bh'] > 0 else "-")  # BH
        view_sheet.cell(row=row_num, column=16, value=data['ae'] if data['ae'] > 0 else "-")  # AE
        view_sheet.cell(row=row_num, column=17, value=data['qa'] if data['qa'] > 0 else "-")  # QA
        view_sheet.cell(row=row_num, column=18, value=data['kw'] if data['kw'] > 0 else "-")  # KW
        view_sheet.cell(row=row_num, column=19, value=data['om'] if data['om'] > 0 else "-")  # OM
...
def main():
    ...
    target_aht = st.number_input("هدف AHT المطلوب", value=300)
    ...
    if st.button("إنشاء التقرير"):
        if raw_overall_file and raw_urdu_file:
            try:
                ...
                # إنشاء التقرير في الذاكرة
                output = BytesIO()
                create_full_report(metrics, target_aht, output)
